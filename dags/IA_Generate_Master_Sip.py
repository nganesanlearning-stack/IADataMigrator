...
CSV_FOLDER = "./data/incomingcsv/"
OUTPUT_FOLDER = "./data/output"
ATTACHMENTS_FOLDER = CSV_FOLDER  # Attachments are now in incomingcsv
SIP_ZIP_FOLDER = "./data/sipzips"

@dag(
    dag_id="IA_Generate_Master_Sip",
    schedule=None,
...
def IA_Generate_Master_Sip(
    ...
    def build_pdi_xml_stream(processed: dict, holding_name: str, root_element: str, child_element: str, output_path: str):
                    ...
                    if path_val:
                        ...
                        if os.path.exists(abs_path):
                            with open(abs_path, "rb") as af:
                                checksum_val = base64.b64encode(hashlib.sha256(af.read()).digest()).decode()
                        f.write('      <Attachment>\n')
                        f.write(f'        <mimetype>{mimetype or "application/octet-stream"}</mimetype>\n')
                        f.write(f'        <Size>{str(size) if size is not None else ""}</Size>\n')
                        f.write(f'        <EASFile>{file_name}</EASFile>\n')
                        f.write(f'        <checksum algorithm="SHA-256" encoding="base64">{checksum_val if checksum_val else ""}</checksum>\n')
                        f.write(f'        <FilePath>/opt/airflow/data/incomingcsv/{file_name}</FilePath>\n')
                        f.write('      </Attachment>\n')
                f.write('    </Attachments>\n')
                f.write(f'  </{child_element}>\n')
    ...
    def package_sips(processed, holding_name, application_name, root_element, child_element, records_per_sip, max_sip_size_kb):
        ...
        for sip_idx, sip_rows in enumerate(clusters):
            ...
            pdi_written = build_pdi_xml_stream(sip_processed, holding_name, root_element, child_element, pdi_xml_path)
...
IA_Generate_Master_Sip = IA_Generate_Master_Sip()